...
def train_val(model, params, train_loader, val_loader, device='cpu'):
    ...
    with open(f'./train_val_log/{model_name}.txt', 'w') as f:
        ...
        for epoch in range(1, num_epochs+1):
            ...
            model.train()
            train_loss, train_acc = loss_epoch(model, model_name, loss_func, train_loader, opt, device)
            loss_history['train'].append(train_loss)
            acc_history['train'].append(train_acc)

            model.eval()
            with torch.no_grad():
                val_loss, val_acc = loss_epoch(model, model_name, loss_func, val_loader, device=device)
